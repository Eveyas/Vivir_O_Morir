...
public class EnemyMoverFantasma : MonoBehaviour
{
    [Header("Movimiento")] 
    ...
    void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        if (knockable != null)
        {
            ...
            // Paralizar al jugador 3 segundos después del knockback (solo para el fantasma)
            var player = collision.collider.GetComponentInParent<Player1_Movimiento>();
            if (player != null)
            {
                player.Aturdir(3f);   // ← paraliza al jugador 3 segundos
            }
